...
1. [概要](#概要)
2. [仕組みの全体像](#仕組みの全体像)
3. [認証：OAuth 1.0a Token-Based Authentication (TBA)](#認証oauth-10a-token-based-authentication-tba)
4. [NetSuiteでの事前設定](#netsuiteでの事前設定)
5. [ローカル環境のセットアップ](#ローカル環境のセットアップ)
6. [メタデータの取得（エクスポート）](#メタデータの取得エクスポート)
7. [メタデータの検索・活用](#メタデータの検索活用)
8. [出力データの構造](#出力データの構造)
9. [対応レコードタイプ（SuiteQLマッピング済み）](#対応レコードタイプsuiteqlマッピング済み)
10. [よくあるエラー](#よくあるエラー)
11. [⚠️ TBA廃止予定についての注意](#️-tba廃止予定についての注意)
...
特にAIを使う時、良く間違ったコードを生成したりして、苦しく感じるときはよくあると思います。
そこで解決法ですが、
