...
スクリプトを実際に動かす場合は `.env.example` をコピーして `.env` を作成し、認証情報を設定する。

```bash
cp development/netsuite-rest-metadata/scripts/.env.example development/netsuite-rest-metadata/scripts/.env
```
...
# 接続テスト
python3 development/netsuite-rest-metadata/scripts/export_metadata_from_rest.py --test-auth

# メタデータ取得
python3 development/netsuite-rest-metadata/scripts/export_metadata_from_rest.py --env SB --select salesorder,customer
```
